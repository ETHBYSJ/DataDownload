...
func ServeContent(w http.ResponseWriter, req *http.Request, name string, modtime time.Time, content io.ReadSeeker) error {
	...
	return serveContent(w, req, name, modtime, sizeFunc, content)
}
...
// var errNotSupport = errors.New("range not support yet")
...
func serveContent(w http.ResponseWriter, r *http.Request, name string, modtime time.Time, sizeFunc func() (int64, error), content io.ReadSeeker) error {
	...
	if done {
		util.Log().Error("checkPreconditions")
	...
	if !haveType {
		...
		if ctype == "" {
			...
			if err != nil {
				...
				util.Log().Error("seeker can't seek")
				return errUnexpected
			}
		}
	...
	util.Log().Error("cannot reach here")
	return errUnexpected
}
